...
def work_is_free(lwork: L.LightningWork):
    ...
    calls at the moment pending status is verified by examining each call history looking for anything call that is
    pending history status.stage is not reliable indicator as there is delay registering new calls status.stage shows
    SUCCEEDED even after 3 more calls are accepted in parallel mode
    """
    status = lwork.status
    state = lwork.state
