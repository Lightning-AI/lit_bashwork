...
def print_status(lwork: L.LightningWork):
    """The statuses shows calls in at most one pending and all running status even if there are 100s in pending, only one
    pending is shown.

    status and statuses before any calls: WorkStatus(stage='not_started', timestamp=1659039787.638122, reason=None,
    message=None, count=1) [] status and statuses after any calls: WorkStatus(stage='pending',
    timestamp=1659039787.651226, reason=None, message=None, count=1) [WorkStatus(stage='pending',
    timestamp=1659039787.651226, reason=None, message=None, count=1)]
    """
...
def work_is_free(lwork: L.LightningWork):
    ...
    this is expensive when a lot of calls accumulate over time work is when there is there is no pending and running calls
    at the moment pending status is verified by examining each call history looking for anything call that is pending
    history status.stage is not reliable indicator as there is delay registering new calls status.stage shows SUCCEEDED
    even after 3 more calls are accepted in parallel mode
    """
    status = lwork.status
    state = lwork.state
