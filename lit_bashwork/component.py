...
class LitBashWork(la.LightningWork):
    def __init__(self, *args, wait_seconds_after_run=10, wait_seconds_after_kill=10, drive_name="lit://lpa", **kwargs):
    ...
    def on_after_run(self):
        ...
        Wrap outputs in Path so they will be available
        """
    ...
    def popen_nowait(self, cmd, **kwargs):
        proc = subprocess.Popen(cmd, shell=True, executable="/bin/bash", close_fds=True, **kwargs)
        self.pid = proc.pid

    def subprocess_call(
        self, cmd, save_stdout=True, exception_on_error=False, venv_name="", wait_for_exit=True, timeout=0, **kwargs
    ):
        """Run the command."""
